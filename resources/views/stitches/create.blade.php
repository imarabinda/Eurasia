...
<div class="row">
   <div class="col-sm-12">
      <div class="page-title-box">
         <h4 class="page-title font-16">Issue New Stitching</h4>
         ...
         <h4 class="page-title">Consignee Details</h4>
                  ...
                  <div class="col-md-5">
                     <div class="form-group">
                        ...
                        <select verify-ignore="false" error-text="@if(count($tailors) > 0) Select Vendor @else Add Tailor First @endif" class="select2 form-control mb-3 custom-select js-states form-control" id="tailor" name="tailor" style="width: 100%; height:36px;">
                           ...
                           <option value="{{$tailor->id}}" data-json="{{json_encode($tailor)}}">{{$tailor->name}}</option>
                           @endforeach
                        </select>
                  ...
                  <div class="col-md-5 ">
                     <div class="form-group">
                        ...
                        <div class="input-group">
                           <input type="text" disabled class="form-control" error-text="Enter vendor name..." placeholder="Enter vendor name" name="vendor_name">
                        </div>
                        <!-- input-group -->
                        <span class="validation-msg" ></span>
                     </div>
                  ...
                  <div class="col-md-5">
                     <div class="form-group">
                        ...
                        <div class="input-group">
                           <input type="text" disabled verify-ignore="true" class="form-control" error-text="Enter vendor gst no..." placeholder="Vendor GST no." name="vendor_gst_no">
                        </div>
                        <!-- input-group -->
                        <span class="validation-msg" ></span>
                     </div>
                  ...
                  <div class="col-md-5">
                     <div class="form-group">
                        ...
                        <div class="input-group">
                           <textarea type="text" disabled class="form-control" error-text="Enter vendor address..." placeholder="Vendor Address" name="vendor_address"></textarea>
                        </div>
                        <!-- input-group -->
                        <span class="validation-msg" ></span>
                     </div>
            ...
            <thead>
                           <tr>
                             <th class="dt-center">{{trans('Title')}}</th>
                              ...
                              <th class="dt-center">{{trans('Emrboidery Stock')}}</th>
                              <th class="dt-center">{{trans('Issue Stitching Stock')}}</th>
...
@if(!empty($items))
                      ...
                      $embroidery_stocks = \App\Models\EmbroideryStock::whereIn('id',$item_ids)->get();
   ...
   @foreach($embroidery_stocks as $embroidery_stock)
         ...
         @endphp
         <tr class="product-row {{$class}}">
          <td><a target="_blank" href="{{route('products.show',$embroidery_stock->product->id)}}">{{$product->name}}</a></td>
          <td><a target="_blank" href="{{route('products.show',$embroidery_stock->product->id)}}">{{$product->code}}</a></td>
          ...
          <td style="width:13%"><input disabled for="stitching_stock[{{$embroidery_stock->id}}]" type="number"  class="form-control" value="{{$embroidery_stock->received_embroidery}}" /></td>
          <td style="width:13%"><input reference-number="stitching_{{$loop->index}}" for="{{$embroidery_stock->id}}" type="number" min="0" max="{{$embroidery_stock->received_embroidery - $embroidery_stock->workables_sum_issued_quantity}}" class="form-control" name="stitching_stock[{{$embroidery_stock->id}}]" value="{{$items[$embroidery_stock->id]}}" /></td>
...
@endif
      ...
      <div class="col-md-12" @if(empty($items)) style="display:none" @endif id="button">
                  <div class="form-group text-center">
                     <input type="button" value="{{trans('Issue Stitching')}}" id="submit-btn" class="btn btn-primary">
                  </div>
               </div>
...
@section('script')
   ...
   $('#product_selector').select2({
     ajax: {
       url: "{{route('relations.embroidery.search')}}",
       dataType: 'json',
       delay: 100,
       method:'POST',
    ...
    $('#product_selector').on('select2:select',function(e){
           var flag=true;
                       ...
                       if ($('input[name="stitching_stock['+e.params.data.position+']"]').length > 0) {
                           swal(
                              'Duplicate',
   ...
   function append_row(data){
    ...
    var newRow = $("<tr class='product-row "+c+"'>");
          ...
          cols += '<td style="width:13%"><input disabled for="stitching_stock['+data.position+']" type="number"  class="form-control" value="'+data.embroidery_stock+'" /></td>';
          cols += '<td style="width:13%"><input reference-number="stitching_'+number+'" max="'+data.embroidery_stock_max+'" for="'+data.position+'" type="number" min="0" max="'+data.embroidery_stock+'" class="form-control" name="stitching_stock['+data.position+']" value="0" /></td>';
          cols += '<td><button type="button" class="rbtnDel btn btn-sm btn-danger">X</button></td>';
          newRow.append(cols);
          $("table.product-list tbody").append(newRow);
   
           $('.product-row').each(function(index){
                var element =$(this).find('input[reference-number="stitching_'+number+'"]');
    ...
    function individual_input(input){
          $(input).on('change', function (e) {
             ...
             if(val > max){
               ...
               validate(false);
               // validate_with_embroidery_stock(this,false);
        });
      ...
      function validate_with_embroidery_stock(element){
            var reference = $(element).attr('name');
            var maximum = $('input[for="'+reference+'"]').val();

            if($(element).val() <= maximum){
                 // validate_with_embroidery_cut_pieces_stock_quantity(element);
            }else{
        ...
        function validate_with_embroidery_cut_pieces_stock_quantity(element,remove=false){
            $.post("{{route('relations.stock_count.embroidery')}}",{unique:$(element).attr('for'),input:$(element).val(),'on':$(element).attr('reference-number'),'remove':remove},function(response){    
                if(!response.success){
                    $(element).val(Math.abs(response.value));
                    $("#product_message").html(response.message);
                }
                if(response.hasOwnProperty('embroidery_stock')){
                   $('input[for="'+$(element).attr("name")+'"]').val(response.embroidery_stock)
                   $(element).attr('max',Math.abs(response.embroidery_stock));
                  }
            })
        }
   ...
   var input = $(this).parent().prev().find('input');
        // validate_with_embroidery_cut_pieces_stock_quantity(input,true);
                 ...
                 $.LoadingOverlay("show");
                      ...
                      $.ajax({
                          ...
                          success:function(response){
                            ...
                            if(response.success){
                               swal({
                                             title: 'Stitching issued successfully!',
                                             text: 'redirecting to manage stitches.',
                                             type: 'success',
