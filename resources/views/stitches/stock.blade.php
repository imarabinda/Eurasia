                                            ...
                                            <div class="table-rep-plugin">
                                                <div class="table-responsive mb-0" data-pattern="priority-columns">
                                                    <table id="stock-data-table" class="table table-bordered" style="width: 100%">
                                                        <thead>
                                                            <tr>
                                                                @can(['shipment-add-product','shipment-create'])
...
@section('script')
        ...
        var table = $('#stock-data-table').DataTable( {
            ...
            "serverMethod": 'post',
            "ajax":{
                url:"<?php echo route('stitches.stock.list'); ?>",
                data:{
            ...
            @endphp
                ...
                {"data": "size","searchable":false,"name":'product.size_id'},
                {"data": "remaining"},
 ...
 @can(['shipment-add-product','shipment-create'])
                 ...
                 
                {
                    ...
                    text: '{{trans("Excel")}}',
                     action: function(e, dt, node, config ){
                         download('{{route('export.final_stocks')}}',{
                            ids:selected_data
                        });   
         ...
         initComplete:function(){
            ...
            $(".bottombuttons").html('<div class="col-md-4 mt-3"><div class="form-group"><button type="button" disabled class="btn btn-secondary waves-effect btn-primary" id="submit-btn"><?php if(isset($id)){
                if(Auth::user()->can('shipment-add-product')){
                    echo 'Add Selected';
                }
         }else{
             if(Auth::user()->can('shipment-create')){    
             echo 'Issue Shipment';
            }
         }?></button></div></div>');
         @endif
...
function format ( key,d ) {
    var input_name = 'issue_shipment['+key+']';
    var val = 0;
    ...
    $('#stock-data-table tbody').on('change','input[type="checkbox"]',function(){
        ...
        var input_name = 'input[name="issue_shipment['+key+']"';
    ...
    function validate(show = false){
      ...
      function init_listeners(){
            ...
            $("#submit-btn").click(function(e){
              e.preventDefault();
              $.LoadingOverlay("show"); 
              $.post('{{route('shipments.bucket')}}',{items:selected_data<?php
              if(isset($id)){
                  echo ',id:'.$id;
              }
              ?>},function(response){
